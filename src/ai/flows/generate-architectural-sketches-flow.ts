...
1.  **Концептуальная архитектура:**
    ...
    *   Подпиши каждый блок и, если возможно, тип взаимодействия на стрелках (например, "API вызов", "Поток данных").
...
2.  **Архитектура приложения (Прикладная архитектура):**
    ...
    *   Используй стандартные архитектурные обозначения.

Убедись, что обе диаграммы четко разделены (например, горизонтальной линией или достаточным пространством) и подписаны заголовками "Концептуальная архитектура" и "Архитектура приложения" (или "Прикладная архитектура") внутри единого сгенерированного изображения. Стиль должен быть профессиональным, четким, легко читаемым и подходящим для технической документации. Используй светлый фон для диаграмм и контрастный текст.
В метках на диаграммах используй русский язык, если это уместно исходя из контекста ТЗ.
      `
    });
