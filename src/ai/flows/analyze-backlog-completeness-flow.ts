...
4.  Если поле уже хорошо заполнено, не нужно генерировать для него предложение (оставь соответствующее \`suggested\` поле пустым).
5.  Предоставь краткий комментарий \`analysisNotes\` на русском языке для каждого элемента, объясняя, были ли сделаны предложения и почему, или указывая, что элемент выглядит полным.
...
Пример ожидаемого выходного элемента в массиве \`analyzedItems\` для указанного входа:
\`\`\`json
{
  "id": "row_1",
  "identifiedUserStory": "Как новый пользователь, я хочу зарегистрироваться в системе, чтобы получить доступ к её функциям.",
  "suggestedUserStory": null, // или "" - так как исходная история полная
  "identifiedGoal": null, // или "" - так как исходная цель пустая
  ...
  "suggestedAcceptanceCriteria": null, // или "" - так как исходные критерии полные
